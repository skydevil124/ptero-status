...
module.exports = async function postStatus({ client, panel, nodes }) {
	...
	stats.then(async () => {
		...
		embed.setDescription(desc.replaceAll('{{time}}', format) + '\n**<:node:1207258020589867018> Nodes Stats [' + Math.floor(nodes.length - blacklist) + ']**' + text)
		const EmbedFields = []

		if (client.config.panel_settings.status) {
			let stats = '**<a:status:1207253934012497951> Status:** ' + String(panel.status).replace('true', client.config.status.online).replace('false', client.config.status.offline) + '\n\n'

			if (client.config.panel_settings.users) stats = stats + '<a:users:1207253053510656000> Users: ' + String(panel.total_users).replace('-1', '`Unknown`') + '\n'
			if (client.config.panel_settings.servers) stats = stats + '<:Servers:1207253504012451850> Servers: ' + String(panel.total_servers).replace('-1', '`Unknown`')
